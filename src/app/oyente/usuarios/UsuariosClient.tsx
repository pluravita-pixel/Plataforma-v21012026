...
export function PatientsClient({ initialPatients, psychologistId }: { initialPatients: User[], psychologistId: string }) {
    ...
    const filteredUsers = initialPatients.filter(user =>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {filteredUsers.length > 0 ? (
                                filteredUsers.map((user) => (
                                    <tr key={user.id} className="hover:bg-[#FAF8F5]/50 transition-colors group relative">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-bold text-[#4A3C31]">{user.fullName || 'Sin nombre'}</p>
                                                    <p className="text-xs text-gray-400 font-medium">{user.email}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-sm text-gray-600 line-clamp-1 italic">"{user.reason || "Sin especificar"}"</p>
                                        </td>
                                        <td className="px-6 py-4 text-sm text-gray-500 font-medium">
                                            {new Date(user.lastSession).toLocaleDateString("es-ES", { day: '2-digit', month: 'short' })}
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${user.status === 'Activo' ? 'bg-emerald-50 text-emerald-700' : 'bg-orange-50 text-orange-700'
                                                }`}>
                                                {user.status}
                                            </span>
                                        </td>
                                                        ...
                                                        <div className="absolute right-12 top-8 z-50 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2 animate-in fade-in zoom-in duration-200 origin-top-right">
                                                            ...
                                                            {user.nextAppointmentId && (
                                                                <button
                                                                    onClick={(e) => {
                                                                        e.stopPropagation();
                                                                        setShowCancelModal(user.nextAppointmentId!);
                                                                        setActiveMenu(null);
                                                                    }}
